  ...
  useEffect(() => {
    if (user) {
      getProspects(user.uid).then(res => {
        const filtered = res.filter(p => p.status === 'Proposal Sent' || p.status === 'Negotiation');
        setData(filtered);
        setLoading(false);
      });
    }
  ...
  return (
    <div className="p-8 flex flex-col gap-6 h-full">
      <div className="flex items-center justify-between">
        <div>
          <h1 className="text-2xl font-bold uppercase tracking-tighter text-[#141414]">Active Proposals</h1>
          <p className="mt-1 text-[10px] uppercase font-bold opacity-50 tracking-widest text-[#141414]">Proposals sent and in negotiation.</p>
        </div>
      </div>
