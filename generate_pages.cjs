...
const pageTemplate = (title, desc, emptyMessage, filterLogic) => `
  ...
  useEffect(() => {
    if (user) {
      getProspects(user.uid).then(res => {
        const filtered = res.filter(p => FILTER_LOGIC);
        setData(filtered);
        setLoading(false);
      });
    }
  ...
  return (
    <div className="p-8 flex flex-col gap-6 h-full">
      <div className="flex items-center justify-between">
        <div>
          <h1 className="text-2xl font-bold uppercase tracking-tighter text-[#141414]">${title}</h1>
          <p className="mt-1 text-[10px] uppercase font-bold opacity-50 tracking-widest text-[#141414]">${desc}</p>
        </div>
      </div>
...
const pages = [
  { name: 'FollowUps', title: 'Follow-ups', desc: 'Action items and high priority targets.', empty: 'NO FOLLOW-UPS PENDING.', filter: "p.priority === 'High' && p.status !== 'Won' && p.status !== 'Lost'" },
  { name: 'Calls', title: 'Scheduled Calls', desc: 'Upcoming discovery and closing calls.', empty: 'NO CALLS SCHEDULED.', filter: "p.status === 'Call Scheduled'" },
  { name: 'Proposals', title: 'Active Proposals', desc: 'Proposals sent and in negotiation.', empty: 'NO ACTIVE PROPOSALS.', filter: "p.status === 'Proposal Sent' || p.status === 'Negotiation'" },
  { name: 'Clients', title: 'Active Clients', desc: 'Converted prospects and current clients.', empty: 'NO ACTIVE CLIENTS YET.', filter: "p.status === 'Won'" }
];

pages.forEach(p => {
  let content = pageTemplate(p.title, p.desc, p.empty, p.filter).replace(/PAGE_NAME/g, p.name).replace(/FILTER_LOGIC/g, p.filter);
  fs.writeFileSync(path.join('src/pages', p.name + '.tsx'), content);
});
